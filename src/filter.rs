...
///
/// This function is used by `filter`.
fn access_img_border(src: &Array<u8, Ix2>, x:i32, y:i32, border: BorderType) -> u8 {
...
///
pub fn filter_(src: &Array<u8, Ix2>, kernel: &Array<f64, Ix2>, border: BorderType, 
               out:&mut Array<u8, Ix2>) {
        ...
        for j in 0..width {
            ...
            for ki in 0..kh {
                for kj in 0..kw {
                    ...
                    let sval = access_img_border(src, sx, sy, border) as f64;
...
///                      [2.0, 4.0, 2.0],
///                      [1.0, 2.0, 1.0]]);
///  let target = arr2(&[[1, 3, 4, 3, 1],
///                      [3, 9, 12, 9, 3],
///                      [4, 12, 16, 12, 4],
///                      [3, 9, 12, 9, 3],
///                      [1, 3, 4, 3, 1]]);
///  let output = filter(&A, &kernel, BorderType::Constant(0));
///  assert_eq!(target, output);
/// ```
///
pub fn filter(src: &Array<u8, Ix2>, kernel: &Array<f64, Ix2>, border: BorderType) -> Array<u8, Ix2>{
    let shape = src.shape();
